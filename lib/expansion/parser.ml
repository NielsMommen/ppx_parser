...
let expand_list_seq ~loc ctxt {pc_lhs; pc_guard; pc_rhs} to_match_expr other_cases =
  ...
  | [%pat? [%p? hd] :: [%p? tl]] ->
    ...
    | pat ->
      ...
      match ctxt with
      | Call ->
        let on_no_match_expr = Ast_builder.Default.pexp_match ~loc to_match_expr other_cases in
        let no_match_case = {pc_lhs = [%pat? _]; pc_guard = None; pc_rhs = on_no_match_expr} in
  ...
  | _ -> 
    ctxt, Err.err_expr_node ~loc:pc_lhs.ppat_loc "Expected a case where the left-hand side is a list of patterns.", other_cases
