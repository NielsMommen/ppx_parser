module Calc1 =
  struct
    let left_assoc op elem =
      let rec op_elem x = function%parser
        | [op as t; elem as y; [%l r = op_elem (t x y)]] -> r
        | [] -> x
      in
      function%parser
      | [elem as x; [%l r = op_elem x]] -> r

    let right_assoc op elem =
      let rec op_elem x = function%parser
      | [op as t; elem as y; [%l r = op_elem y]] -> t x r
      | [] -> x
      in
      function%parser 
      | [elem as x; [%l r = op_elem x]] -> r
    ...
    let parse = 
      List.fold_right 
        ...
        (function%parser [(('0' .. '9') as c)] -> float (Char.code c - Char.code '0'))
  end
...
module Calc2 = 
  ...
  [@@deriving show, eq]
    ...
    let rec integer n = function%parser
    ...
    let addop = function%parser
      | [ PLUS ] -> (function (x, y) -> Plus(x, y))
      | [ MINUS ] -> (function (x, y) -> Minus(x, y))

    let multop = function%parser
      | [ TIMES ] -> (function (x, y) -> Mult(x, y))
      | [ DIV ] -> (function (x, y) -> Div(x, y))

    let left_assoc op term =
      let rec rest e1 = function%parser
      | [ op as f; term as e2; [%l e = (rest (f (e1,e2)))] ] -> e
      | [ ] -> e1
      in 
      function%parser 
      | [ term as e1; [%l e2 = (rest e1)] ] -> e2

    let rec expr str = left_assoc addop mult str
    and mult str = left_assoc multop atom str
    and atom = function%parser
      | [ INT n ] -> Int n
      | [ LPAR; expr as e; RPAR ] -> e
  end

let calc1 s = let open Calc1 in s |> Stream.of_string |> parse
let calc2 s = let open Calc2 in s |> Stream.of_string |> lexer |> expr

let mk_test testble calc (input, output) =
  let test () = Alcotest.(check' testble) ~msg:input ~expected:output ~actual:(calc input) in
