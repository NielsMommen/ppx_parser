...
let test_char_codes1, test_char_codes2, test_char_codes3 =
  ...
  ( (fun () ->
      Alcotest.(check' (list int))
        ~msg:"consume spaces get char codes" ~expected ~actual:actual1),
    (fun () ->
      Alcotest.(check' (list int))
        ~msg:"consume spaces get char codes anonymous parser" ~expected
        ~actual:actual2),
    fun () ->
      Alcotest.(check' (list int))
        ~msg:"consume spaces get char codes let parser" ~expected
        ~actual:actual3 )
...
let test_split_tuples () =
  ...
  Alcotest.(check' (pair (list int) (list int)))
    ~msg:"split tuples" ~expected ~actual

type ('a, 'b) record_tuple = { rt_a : 'a; rt_b : 'b } [@@deriving show, eq]
...
let split_rec_tuple_fields =
  let rec p r =
    ...
    | [
        ...
        [%let
          { rt_a = a; rt_b = b }
          (* 1 *) = p { rt_a = rt_a :: r.rt_a; rt_b = rt_b :: r.rt_b }];
      ] ->
        { rt_a = b; rt_b = a }
...
let test_split_rec_tuple_fields () =
  ...
  let lst_formatter =
    Format.pp_print_list (fun fmt i -> Format.pp_print_int fmt i)
  in
  let testbl =
    Alcotest.testable
      (fun fmt r ->
...
let tests =
  ...
  [
    ...
    test_case "consume_spaces_get_char_code_anonymous_parser" `Quick
      test_char_codes2;
