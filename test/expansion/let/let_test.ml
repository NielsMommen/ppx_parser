...
let test_parser_calls_parser () =
  let actual = f [%expr  
    function%parser
    | [op as t; elem as y; [%l r = op_elem (t x y)]] -> r
    | [] -> x
  ]
  in
  let expected = [%expr   
    function ppx____parser____stream____ ->
      match try Some (op ppx____parser____stream____) with Stream.Failure -> None with
      | Some t ->
        let y = try elem ppx____parser____stream____ with Stream.Failure -> [%e raise_err_exn] in
        let r = try (op_elem (t x y)) ppx____parser____stream____ with Stream.Failure -> [%e raise_err_exn] in
        r
      | _ -> x
  ]
  in
...
let test_bind_pats_parser () =
  ...
  function%parser
    ...
    | [ [%let {rt_b; rt_a} = get_rec_tuple_one_two]; 0; [%let Some x = get_some_one] ] -> rt_b + rt_a + x
  ...
  let expected = [%expr   
    function ppx____parser____stream____ ->
      match try Some (get_const_one ppx____parser____stream____) with Stream.Failure -> None with
      | Some 1 ->
        let a, 2 = try get_tuple_one_two ppx____parser____stream____ with Stream.Failure -> [%e raise_err_exn] in
        a
      | _ ->
        (match try Some (get_rec_tuple_one_two ppx____parser____stream____) with Stream.Failure -> None with
        | Some ({rt_b; rt_a}) -> 
          (match [%e peek] with
          | Some 0 ->
            let () = [%e junk] in
            let Some x = try get_some_one ppx____parser____stream____ with Stream.Failure -> [%e raise_err_exn] in
            rt_b + rt_a + x
          | _ -> [%e raise_err_exn])
        | _ ->
          (match [%e peek] with
          | Some 3 ->
            let () = [%e junk] in
            let None = try get_none ppx____parser____stream____ with Stream.Failure -> [%e raise_err_exn] in
            let Int x = try get_int_lit_one ppx____parser____stream____ with Stream.Failure -> [%e raise_err_exn] in
            x
          | Some 4 ->
            let () = [%e junk] in
            let [| a; b; c |] = try get_arr ppx____parser____stream____ with Stream.Failure -> [%e raise_err_exn] in
            let lazy x = try get_lazy ppx____parser____stream____ with Stream.Failure -> [%e raise_err_exn] in 
            a + b + c + x
          | Some 5 ->
            let () = [%e junk] in
            (match [%e peek] with
