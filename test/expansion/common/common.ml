open Ppxlib
let testble = Alcotest.testable (fun fmt s -> Format.pp_print_string fmt s) (fun a b -> String.equal a b)
...
let parser_ext_pat = Ast_pattern.(pexp_extension (extension (string "parser") Ppx_parser.ppx_parser_pat))
